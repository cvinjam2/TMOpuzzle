...
export class PriceQueryFacade {
  ...
  fetchQuote(symbol: string, period: string, fromDateNumeric: number, toDateNumeric: number) {
    this.store.dispatch(new FetchPriceQuery(symbol, period, fromDateNumeric, toDateNumeric));
  }
}
